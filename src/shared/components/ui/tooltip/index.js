...
//   setArrowPosition(arrow, finalPosition, anchorOffset - 4);
...
function ensureTooltip() {
  ...
  arrowEl.className = "ui-tooltip-arrow";
...
export function showTooltip(e, message, position) {
  ...
  if (finalPosition === "top" || finalPosition === "bottom") {
    ...
    if (overflowLeft > 0) computedLeft += overflowLeft;
    else if (overflowRight > 0) computedLeft -= overflowRight;
  ...
  if (finalPosition === "left" || finalPosition === "right") {
    ...
    if (overflowTop > 0) computedTop += overflowTop;
    else if (overflowBottom > 0) computedTop -= overflowBottom;
    ...
    anchorOffset = centerY - top;
  }

  tooltipEl.style.top = `${top}px`;
  tooltipEl.style.left = `${left}px`;

  arrowEl.dataset.placement = finalPosition;
